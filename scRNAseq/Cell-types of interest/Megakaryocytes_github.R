...
Dif_Exp<-rbind(Cluster.response1, Cluster.response2, Cluster.response3, Cluster.response4, Cluster.response5,
               Cluster.response6, Cluster.response7)

# Go enrichment analysis (example)
table<-subset(Dif_Exp, Dif_Exp$signal %in% 'Upegulated in 1')
UnderGenes=rownames(table)
anno <- AnnotationDbi::select(org.Hs.eg.db, 
                              keys=rownames(Megakaryocytes), 
                              columns=c("SYMBOL","SYMBOL", "GENENAME", "ENSEMBL"),
                              keytype="SYMBOL")
anSig1 <- as.data.frame(subset(anno, SYMBOL %in% UnderGenes))
...
for(i in 1:length(topGOResults$GO.ID)){
  ...
  genes_in_cat <- intersect(as.vector(genes), as.vector(UnderGenes))
  #print(genes_in_cat)
...
# select top 10 genes
topGOResults_top <- topGOResults_top[order(topGOResults_top$Fisher.classic),] 
...
## Validation cohort2
Megakaryocytes_cohort2 <- readRDS("Megakaryocytes_cohort2.rds")
cell_counts<-read.csv2('~/Documents/IKMB/scRNAseq/ProjectC/Github_scripts/Prop_Megakaryocytes_cohort2_github.csv')

Idents(Megakaryocytes_cohort2)<- Megakaryocytes_cohort2$hemato.labels
Megakaryocytes_cohort2<-subset(Megakaryocytes_cohort2, idents='Megakaryocytes')

# Cohort2 megakaryocytes proportion
cell_counts$Mega_rel<-(cell_counts$Megakaryocytes/cell_counts$Total.cells)*100
p <- ggplot(cell_counts, aes(x=Group, y=Mega_rel)) 
p <- p+  geom_boxplot(aes(fill=Group))
p <- p + geom_dotplot(binaxis='y', stackdir='center')
p <- p + theme_bw() + theme(axis.text.y = element_text(hjust = 1, size=12, color = "black"), 
                            axis.text.x = element_text(size=14, color = "black"), 
                            axis.title = element_text(size = 20))
p <- p + scale_fill_manual(values =c('#f0f0f0','#bdbdbd','#636363') )
p
...
VlnPlot(Megakaryocytes, features =c('ODC1','TGFB1',"IFITM3", 'CD27', 'PKM', 'FCER1G','TREML1', 'ITGA2B', 'IFITM2', 'IFI27', 'TNFSF13'),  pt.size = 0,ncol = 6,
        cols = c("#A7A9AC", '#E84F8C', "#7031B9", "#A04E9E",'#E65826',"#F99B1C",'#FDC077','#51BBFE'))
# Cohort 2
Idents(Megakaryocytes_cohort2)<- Megakaryocytes_cohort2$group_per_sample
my_levels<- c("control","mild",'severe')
Idents(Megakaryocytes_cohort2) <- factor(Idents(Megakaryocytes_cohort2), levels= my_levels)
VlnPlot(Megakaryocytes_cohort2, features =c('ODC1','TGFB1',"IFITM3", 'CD27', 'PKM', 'FCER1G','TREML1', 'ITGA2B', 'IFITM2', 'IFI27', 'TNFSF13'),  pt.size = 0,ncol = 6,
        cols = c('#f0f0f0','#bdbdbd','#636363'))
